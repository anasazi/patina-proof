...
data Layout (#a : ℕ) : Set where
  ...
  ~ : Value (Fin #a) → Layout #a
  & : Value (Route #a) → Layout #a
...
data _⊢_⇒_ {#a} : Heap #a → Route #a → Layout #a → Set where
  ...
  *~ : ∀ {H r α} → H ⊢ r ⇒ ~ (val α) → H ⊢ * r ⇒ contents (H ! α)
  *& : ∀ {H r r′ l} → H ⊢ r ⇒ & (val r′) → H ⊢ r′ ⇒ l → H ⊢ * r ⇒ l
...
test-deref-1 = alloc
test-deref-2 : test-H ⊢ alloc (fin 1) ⇒ ~ (val (fin 12))
test-deref-2 = alloc
...
test-write-1 = alloc
test-write-2 : ([ alloc (~ (val (fin 1))) ,, alloc (int (val 1)) ])
               ⊢ alloc (fin 0) ≔ ~ void
               ⇒ ([ alloc (~ void) ,, alloc (int (val 1)) ])
test-write-2 = alloc
test-write-3 : ([ alloc (~ (val (fin 1))) ,, alloc (int (val 1)) ])
               ⊢ * (alloc (fin 0)) ≔ int void
               ⇒ ([ alloc (~ (val (fin 1))) ,, alloc (int void) ])
test-write-3 = *~ alloc
test-write-4 : ([ alloc (& (val (alloc (fin 1)))) ,, alloc (int (val 1)) ])
               ⊢ alloc (fin 0) ≔ & void
               ⇒ ([ alloc (& void) ,, alloc (int (val 1)) ])
test-write-4 = alloc
test-write-5 : ([ alloc (& (val (alloc (fin 1)))) ,, alloc (int (val 1)) ])
               ⊢ * (alloc (fin 0)) ≔ int void
               ⇒ ([ alloc (& (val (alloc (fin 1)))) ,, alloc (int void) ])
...
data _⊢_∶_layout {#a} (σ : Vec Type #a) : Layout #a → Type → Set where
  ...
  ~ : ∀ {α} → σ ⊢ ~ (val α) ∶ ~ (σ ! α) layout
  ~⊥ : ∀ {τ} → σ ⊢ ~ void ∶ ~ τ layout
  & : ∀ {r τ} → σ ⊢ r ∶ τ route → σ ⊢ & (val r) ∶ & τ layout
  &⊥ : ∀ {τ} → σ ⊢ & void ∶ & τ layout
